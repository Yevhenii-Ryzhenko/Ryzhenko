...
# 0-firstname, 1-lastname,2-gender,3-hobbies,4-street address,5-landmark,6-area,7-city,8-state,9-pincode
def get_address():
	full_addr = []
	addrParam = ['street', 'landmark', 'area', 'city', 'state', 'pincode']
	# діапазон колонок змінено на коректний
	for i in range(4,10):
		allAddrs = []
		with open(full_path('data.csv'), 'r') as f:
			...
			for addr in addrFile:
				# перевіряємо чи існує потрібна колонка і видаляємо пробіли та порожні значення
				if len(addr) > i and addr[i].strip():
					allAddrs.append(addr[i].strip())
		# додаємо значення тільки, якщо воно існує
		if allAddrs:
